...
exports.updateOrderTracking = exports.claimGuestOrders = exports.cancelOrder = exports.updateOrderStatus = exports.getAllOrders = exports.getUserOrders = exports.getOrderById = exports.createOrder = void 0;
...
const createOrder = async (req, res) => {
    ...
    try {
        ...
        for (const item of cart.items) {
            ...
            totalAmount = totalAmount.plus(new client_1.Prisma.Decimal(item.product.prezzo).times(item.quantity));
            orderItemsData.push({
        ...
        console.log("🔧 DEBUG: Iniziando processo invio email per ordine:", createdOrder.id);
        try {
            ...
            if (!createdOrder.user) {
                ...
                // Email al cliente
                console.log(`📧 Tentativo invio email conferma ordine a: ${orderData.customerEmail}`);
                const customerEmailSent = await emailService_1.default.sendOrderConfirmationEmail(orderData);
                if (customerEmailSent) {
                    console.log(`✅ Email conferma ordine inviata al cliente: ${orderData.customerEmail}`);
                }
                else {
                    console.log(`⚠️ Fallimento invio email conferma ordine al cliente: ${orderData.customerEmail}`);
                }
                // Email all'admin
        ...
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError) {
            ...
            if (error.code === "P2025") {
                res.status(404).json({
                    message: "Uno o più record necessari (es. indirizzo, prodotto) non sono stati trovati.",
...
const getUserOrders = async (req, res) => {
    ...
    try {
        ...
        console.log(`📦 Recuperati ${orders.length} ordini per utente ${userId} (email: ${currentUser.email})`);
        console.log("🔍 Ordini trovati:", orders.map((o) => ({
            id: o.id,
            userId: o.userId,
...
const getAllOrders = async (req, res) => {
    ...
    const { page = "1", limit = "10", status, userId: queryUserId, sortBy = "createdAt", sortOrder = "desc", } = req.query;
...
const updateOrderStatus = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (customerEmail) {
                ...
                const orderData = {
                    ...
                    customerName,
                    customerEmail: customerEmail, // Safe cast perché abbiamo il check sopra
                ...
                if (status === client_1.OrderStatus.SHIPPED) {
                    ...
                    console.log(`📧 ${logPrefix}Tentativo invio email ordine spedito a: ${orderData.customerEmail}`);
                    const emailSent = await emailService_1.default.sendOrderShippedEmail(orderDataWithTracking);
                    if (emailSent) {
                        console.log(`✅ ${logPrefix}Email ordine spedito inviata al cliente: ${orderData.customerEmail}`);
                    }
                    else {
                        console.log(`⚠️ ${logPrefix}Fallimento invio email ordine spedito al cliente: ${orderData.customerEmail}`);
                    }
                }
                else if (status === client_1.OrderStatus.CANCELLED) {
                    const logPrefix = updatedOrder.user ? "" : "[GUEST] ";
                    console.log(`📧 ${logPrefix}Tentativo invio email ordine cancellato a: ${orderData.customerEmail}`);
                    const emailSent = await emailService_1.default.sendOrderCancelledEmail(orderData);
                    if (emailSent) {
                        console.log(`✅ ${logPrefix}Email ordine cancellato inviata al cliente: ${orderData.customerEmail}`);
                    }
                    else {
                        console.log(`⚠️ ${logPrefix}Fallimento invio email ordine cancellato al cliente: ${orderData.customerEmail}`);
                    }
                    // Invia anche notifica all'admin
                    console.log(`📧 Tentativo invio notifica cancellazione all'admin`);
                    const adminEmailSent = await emailService_1.default.sendOrderCancelledNotificationToAdmin(orderData);
                    if (adminEmailSent) {
                ...
                console.error("❌ Nessuna email trovata per l'ordine:", updatedOrder.id, "- User email:", updatedOrder.user?.email, "- Guest email:", updatedOrder.guestEmail);
            }
        }
        catch (emailError) {
            console.error("❌ Errore durante invio email aggiornamento ordine:", emailError);
        ...
        res.status(500).json({
            message: "Errore interno del server durante l'aggiornamento dello stato dell'ordine.",
            error: error.message,
        });
        return;
    }
    ...
    try {
        ...
        if (userRole !== client_1.Role.ADMIN) {
            ...
            if (now > cancelDeadline) {
                res.status(400).json({
                    message: "Il periodo di cancellazione gratuita (24 ore) è scaduto. Contatta l'assistenza clienti per richiedere la cancellazione.",
                });
                return;
            }
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (order.paymentIntentId) {
                try {
                    console.log(`💳 Processando rimborso Stripe per ordine ${orderId}, PaymentIntent: ${order.paymentIntentId}`);
                    refundResult = await stripe.refunds.create({
                        payment_intent: order.paymentIntentId,
                        reason: "requested_by_customer",
                        metadata: {
                            orderId: orderId.toString(),
                            cancelledBy: userRole === client_1.Role.ADMIN ? "admin" : "customer",
                            userId: userId.toString(),
                        },
                    });
                    console.log(`✅ Rimborso Stripe creato con successo: ${refundResult.id}`);
                    ...
                    console.error(`❌ Errore durante il rimborso Stripe per ordine ${orderId}:`, stripeError);
        ...
        try {
            ...
            if (customerEmail) {
                const orderData = {
                    ...
                    cancelReason: userRole === client_1.Role.ADMIN
                        ? "Cancellato dall'amministratore"
                        : "Richiesto dal cliente",
                };
                const logPrefix = order.user ? "" : "[GUEST] ";
                console.log(`📧 ${logPrefix}Tentativo invio email ordine cancellato a: ${customerEmail}`);
                const emailSent = await emailService_1.default.sendOrderCancelledEmail(orderData);
                if (emailSent) {
                    console.log(`✅ ${logPrefix}Email ordine cancellato inviata al cliente: ${customerEmail}`);
                }
                else {
                    console.log(`⚠️ ${logPrefix}Fallimento invio email ordine cancellato al cliente: ${customerEmail}`);
                }
            ...
            console.error("❌ Errore durante invio email cancellazione ordine:", emailError);
        }
        ...
        res.status(500).json({
            message: "Errore interno del server durante la cancellazione dell'ordine.",
            error: error.message,
        });
        return;
    }
...
const claimGuestOrders = async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ message: "Utente non autorizzato." });
        return;
    ...
    try {
        ...
        console.log(`🔄 Trovati ${guestOrders.length} ordini guest per email ${currentUser.email}`);
        ...
        console.log(`✅ Reclamati ${updateResult.count} ordini per utente ${userId}`);
        res.json({
        ...
        console.error(`Errore nel reclamare ordini guest per utente ${userId}:`, error);
        res.status(500).json({
...
exports.claimGuestOrders = claimGuestOrders;
...
const updateOrderTracking = async (req, res) => {
    ...
    try {
        ...
        if (updatedOrder.status === client_1.OrderStatus.SHIPPED) {
            const customerEmail = updatedOrder.user?.email || updatedOrder.guestEmail;
            if (customerEmail) {
                const orderData = {
                    orderId: updatedOrder.id.toString(),
                ...
                const logPrefix = updatedOrder.user ? "" : "[GUEST] ";
                console.log(`📧 ${logPrefix}Tentativo invio email tracking number a: ${customerEmail}`);
                const emailSent = await emailService_1.default.sendOrderShippedEmail(orderData);
                if (emailSent) {
                    console.log(`✅ ${logPrefix}Email tracking number inviata al cliente: ${customerEmail}`);
                }
                else {
                    console.log(`⚠️ ${logPrefix}Fallimento invio email tracking number al cliente: ${customerEmail}`);
                }
            }
        }
        res.json({
        ...
        console.error(`Errore nell'aggiornamento del tracking number dell'ordine ${orderId}:`, error);
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError &&
            error.code === "P2025") {
            res
                .status(404)
        ...
        res.status(500).json({
            message: "Errore interno del server durante l'aggiornamento del tracking number.",
            error: error.message,
        });
        return;
    }
