...
const getAllProducts = async (req, res) => {
    const { page = "1", limit = "10", categoryId, minPrice, maxPrice, sortBy = "createdAt", // e.g., titolo, prezzo, createdAt
    ...
    const where = {};
    if (categoryId) {
        where.categoria = { some: { id: parseInt(categoryId) } };
    }
    if (minPrice || maxPrice) {
        where.prezzo = {};
        if (minPrice) {
            where.prezzo.gte = parseFloat(minPrice);
        }
        if (maxPrice) {
            where.prezzo.lte = parseFloat(maxPrice);
        }
    }
    if (search) {
        where.OR = [
...
const createProduct = async (req, res) => {
    ...
    const { titolo, descrizione, prezzo, immagine, categoriaId, stock, varianti, } = req.body;
    ...
    try {
        ...
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError) {
            if (error.code === "P2002" && error.meta?.target) {
                res.status(409).json({
                    message: `Conflitto: il campo ${Array.isArray(error.meta.target)
                        ? error.meta.target.join(", ")
                        : error.meta.target} deve essere univoco.`,
                });
                return;
            }
            if (error.code === "P2003") {
...
const updateProduct = async (req, res) => {
    ...
    try {
        const dataToUpdate = {};
        if (titolo !== undefined)
            dataToUpdate.titolo = titolo;
        if (descrizione !== undefined)
            dataToUpdate.descrizione = descrizione;
        if (prezzo !== undefined)
            dataToUpdate.prezzo = parseFloat(prezzo);
        if (immagine !== undefined)
            dataToUpdate.immagine = immagine;
                    ...
                    where: {
                        typeId: {
                            in: existingVariantTypes
                                .filter((t) => !typeIdsToKeep.includes(t.id))
                                .map((t) => t.id),
                        },
                    },
                });
...
const deleteProduct = async (req, res) => {
    ...
    try {
        ...
        if (orderItemsCount > 0) {
            res.status(409).json({
                message: "Impossibile eliminare il prodotto. È presente in ordini esistenti. Per motivi di conformità fiscale e storico vendite, i prodotti ordinati non possono essere eliminati.",
                orderItemsCount,
            });
            return;
        }
        ...
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError) {
            ...
            if (error.code === "P2014") {
                res.status(409).json({
                    message: "Impossibile eliminare il prodotto. Esistono record correlati (es. ordini, carrelli) che lo referenziano.",
                });
                return;
            }
        }
